...
#6 save plots NOTE: these plots were not used in the manuscript, so the code was removed for clarity.
...
for(i in 1:100){
    ...
    model_predictions[[i]] <- PredFunction(ModelList = AllIterations.onevEach,TestData = wholeData_thinned, Indices = dataSplits$samples[[i]], classes.df = classInformation)
}
