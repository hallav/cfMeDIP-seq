...
if(strtoi(args[13])==0){
  DataMatrix_training <- log2(DataMatrix_training * 0.3 + 1e-6)
}else{
  DataMatrix_training <- log2(DataMatrix_training * 0.3 + 0.5)
}



val.rownames<-rownames(DataMatrix_validation)

#Do the transformation to validation data
if(strtoi(args[13])==0){
  DataMatrix_validation <- log2(DataMatrix_validation * 0.3 + 1e-6)
}else{
  DataMatrix_validation <- log2(DataMatrix_validation * 0.3 + 0.5)
}


print(dim(DataMatrix_validation))

#From MachineLearning_Final.html
...
for(i in 1:N_SPLITS){
  ...
  ValData <- DataMatrix_validation[match(Features, val.rownames),]
...
save(ROC_list, file = paste(args[12],"/data_for_ROC_validation_",args[15],".RData",sep=""),compress="xz")
