#Script for running ISPCA for nonthinned data set. 
...
#8 binarize classes before ISPCA (yes=1, no=0)
...
if (strtoi(args[6])==1){
  #ISPCA using class-specific DMRs.
  PCA_result_list <- list()
  for(i in 1:length(AllClasses)){
    PCA_results <- ISPCA_allWindows(DataMatrix,classes.df = dataSplits$df, Indices = dataSplits$samples[[ID]], FeatureList=FeatureList, class=i, transform=1,NC=length(dataSplits$samples[[ID]])-1,NORMALIZE=normalize_components)
    PCA_result_list[[i]] <- PCA_results
  }
    
}else{
  #ISPCA without using DMRs. Note that class is placeholder
  PCA_results <- ISPCA_allWindows(DataMatrix, classes.df = dataSplits$df, Indices = dataSplits$samples[[ID]], FeatureList=NULL, class=10, transform=1,NC=length(dataSplits$samples[[ID]])-1,NORMALIZE=normalize_components)
