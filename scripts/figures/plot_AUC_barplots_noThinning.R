#This script is for making median AUC barplots for the non-thinned data
#Note that the file and folder paths should be defined before running this script
...
FIGFOLDER<-### Folder where the figures are stored
...
#Do the barplots for discovery cohort
INPUTFOLDER=### Folder where the discovery cohort AUC values have been stored
...
#Do the barplots for validation cohort
N_classes<-4
CLASSNAMES<-c("AML","LUC","PDAC","Normal")

INPUTFOLDER=### Folder where the validation cohort AUC values have been stored
...
LR_DMR_Fisher_results <- AUC.median.df
load(paste(INPUTFOLDER,"/validation_AUC_values_LR_PCA.RData",sep=""))
LR_PCA_results <- AUC.median.df
...
plot_list_AUPRC[[index]]<-ggplot(data=pr.auc.data,aes(x=name, y=auc_median, fill=Method)) +
  ...
  p5<-ggplot(data=pr.auc.data,aes(x=name, y=auc_median, fill=Method)) +
    geom_bar(stat="identity", color="black", position=position_dodge(),size=0.2)+
    scale_fill_manual(values = getPalette(N_methods))+
    geom_errorbar(aes(ymin=auc_25, ymax=auc_75), position=position_dodge(.9), width=0.6, colour="orange",alpha=1, size=0.6)+
    ylim(0,1)+
  ...
  legend_barplot_validation <- get_legend(p5)
...
FIGNAME<-"noThinning_AUROC_barplots"
...
ggdraw(plot_grid(barplot_grid, legend_barplot_validation, ncol=2,scale = c(1,1),rel_widths=c(6,1)))
dev.off()
embed_fonts(paste(FIGFOLDER,"/",FIGNAME,".pdf",sep=""), outfile = paste(FIGFOLDER,"/",FIGNAME,"_embed.pdf",sep=""))


FIGNAME2<-"noThinning_AUPRC_barplots"
...
pdf(file=paste(FIGFOLDER,"/",FIGNAME2,".pdf",sep=""),
    width = WIDTH_PDF, height = HEIGHT_PDF)
ggdraw(plot_grid(barplot_grid2, legend_barplot_validation, ncol=2,scale = c(1,1),rel_widths=c(6,1)))
dev.off()
