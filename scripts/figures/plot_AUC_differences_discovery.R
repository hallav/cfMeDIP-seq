#Script for plotting the median AUC differences for the dicovery cohort
#Note that the file and folder paths have to be defined before running this script
...
if(THINNING==6){
  ...
  ylim(-0.25,0.25)+
  theme(axis.text.x = element_text(angle = 45),text = element_text(size=7))+
  theme(legend.key.size = unit(0.5, 'lines'),legend.title = element_blank())+
...
FIG_ID="truncated_170mm_withMeans"
WIDTH_PDF<-6.69
HEIGHT_PDF<-5
...
pdf(file=paste(FIGFOLDER,"/discovery_AUPRC_comparison_diffToGlmnet_",FIG_ID,".pdf",sep=""),width = WIDTH_PDF, height = HEIGHT_PDF)
...
#plot and save the figure
pdf(file=paste(FIGFOLDER,"/discovery_AUROC_comparison_diffToGlmnet_",FIG_ID,".pdf",sep=""),width = WIDTH_PDF, height = HEIGHT_PDF)
...
embed_fonts(paste(FIGFOLDER,"/discovery_AUPRC_comparison_diffToGlmnet_",FIG_ID,".pdf",sep=""), outfile = paste(FIGFOLDER,"/discovery_AUPRC_comparison_diffToGlmnet_",FIG_ID,"_embed.pdf",sep=""))
embed_fonts(paste(FIGFOLDER,"/discovery_AUROC_comparison_diffToGlmnet_",FIG_ID,".pdf",sep=""), outfile = paste(FIGFOLDER,"/discovery_AUROC_comparison_diffToGlmnet_",FIG_ID,"_embed.pdf",sep=""))
