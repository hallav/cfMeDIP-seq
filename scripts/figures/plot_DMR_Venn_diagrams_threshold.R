...
#Note that file/folder paths must be defined before running this scripts.
...
rm(FeatureList)
  ...
  for(class in 1:8){
    if(i==1){
      ...
      DMRList_M1[[class]] <-c(DMRList_M1[[class]],DMR1[[class]])
      DMRList_M2[[class]] <-c(DMRList_M2[[class]],DMR3[[class]])
      DMRList_M3[[class]] <-c(DMRList_M3[[class]],DMR3[[class]])
    } 
...
print("CLASSNAMES object")
print(CLASSNAMES)

for(class in 1:8){

  tmp <- rle(sort(DMRList_M1[[class]]))
  DMRList_M1[[class]] <-tmp$values[tmp$lengths>=50]

  tmp <- rle(sort(DMRList_M2[[class]]))
  DMRList_M2[[class]] <-tmp$values[tmp$lengths>=50]

  tmp <- rle(sort(DMRList_M3[[class]]))
  DMRList_M3[[class]] <-tmp$values[tmp$lengths>=50]
}
...
FIG_ID <-"allThinnings_2cols_newColors_fixedNames_170mm"
...
pdf(file=paste(FIGFOLDER,"/venn_diagram_DMR_overlap_threshold_allClasses_",FIG_ID,"_pdf.pdf",sep=""),
    width = WIDTH_PDF, height = HEIGHT_PDF)
plot_grid(top_row,middle_row,bottom_row,labels = "AUTO",label_size = 9, ncol = 1,scale = 0.9)
dev.off()
embed_fonts(paste(FIGFOLDER,"/venn_diagram_DMR_overlap_threshold_allClasses_",FIG_ID,"_pdf.pdf",sep=""), outfile = paste(FIGFOLDER,"/venn_diagram_DMR_overlap_threshold_allClasses_",FIG_ID,"_pdf_embed.pdf",sep=""))
