...
#Braindata does not have to be normalized, as it is given as RPKM
...
if (strtoi(args[6])==1){
  for(i in 1:length(AllClasses)){
    PCA_results <- ISPCA_allWindows(BrainData.matrix,classes.df = dataSplits$df, Indices = dataSplits$samples[[ID]], FeatureList=FeatureList, class=i, transform=1,NC=length(dataSplits$samples[[ID]])-1,NORMALIZE=normalize_components,binarize=1)
    PCA_result_list[[i]] <- PCA_results
  }
    
}else{
  for(i in 1:length(AllClasses)){
    PCA_results <- ISPCA_allWindows(BrainData.matrix, classes.df = dataSplits$df, Indices = dataSplits$samples[[ID]], FeatureList=NULL, class=i, transform=1,NC=length(dataSplits$samples[[ID]])-1,NORMALIZE=normalize_components,binarize=1)
    PCA_result_list[[i]] <- PCA_results
  }
