...
if (($#==1)); then
    ...
    PCAFILE=".../results/DMRs/ISPCA_multiclass/ISPCA_DMR_results_dataSplit_"$ID".RData"
    OUTPUTFOLDER=".../results/LR_RHS/LR_RHS_ISPCA_multiclass"
    ...
    srun Rscript --no-save train_LR_RHS_ISPCA_intracranial_tumors.R $ID $PCAFILE $INPUTFILE "$OUTPUTFOLDER"/LR_RHS_ID $CLASS $DATASPLITFILE $P0 $N_COMPONENTS $PLOTFOLDER 0
