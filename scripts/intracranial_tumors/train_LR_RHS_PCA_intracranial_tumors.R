...
ourModel.onevEach <- train_LR_RHS_model_PCA_features(Mat = BrainData.matrix, classes.df = dataSplits$df, Indices = dataSplits$samples[[ID]], index=CLASS, parameter_P0=strtoi(args[7]), pca_results=PCA_result_list[[CLASS]], N_components=strtoi(args[8]), pairsPlot=pairsplot, FigFolder=args[9], dataSplitID=ID, debug=debugplots, ISPCA=0, normalize_pca=1)
