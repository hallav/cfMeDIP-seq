...
if (($#==1)); then
    ...
    PCAFILE="$RESULTFOLDER"/DMRs/noThinning/PCA/PCA_results_dataSplit_"$ID".RData
    OUTPUTFOLDER="$RESULTFOLDER"/LR_RHS/noThinning/LR_RHS_PCA
    ...
    srun Rscript --no-save train_LR_RHS_PCA_noThinning.R $ID $PCAFILE $INPUTFILE "$OUTPUTFOLDER"/LR_RHS_ID $CLASS $DATASPLITFILE $P0 $N_COMPONENTS $PLOTFOLDER
