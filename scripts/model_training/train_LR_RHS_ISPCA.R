...
ourModel.onevEach <- train_LR_RHS_model_PCA_features(Mat = wholeData_thinned, classInformation = dataSplits$df, TrainIndices = dataSplits$samples[[ID]], index=CLASS, parameter_P0=strtoi(args[7]), pca_results=PCA_OBJECT, N_components=strtoi(args[8]), pairsPlot=pairsplot, FigFolder=args[9], dataSplitID=ID,debug=0,ISPCA=1)
